        ...
        if (res.ok) {
          ...
          if (data?.authenticated && data?.user) {
            ...
            if (emailVerified) {
              ...
              if (isAdmin) navigate('/admin'); else navigate('/map');
            }
          }
        }
      } catch (err) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const serverMsg = body?.message || 'ไม่สามารถเข้าสู่ระบบได้ กรุณาลองใหม่';
        setMsg(serverMsg);
        setIsLoading(false);
        return;
      }
      ...
      if (error?.code === 'auth/user-not-found') errorMessage = 'ไม่พบผู้ใช้งาน กรุณาตรวจสอบอีเมลหรือเบอร์โทรศัพท์';
      else if (error?.code === 'auth/wrong-password') errorMessage = 'รหัสผ่านไม่ถูกต้อง กรุณาตรวจสอบอีกครั้ง';
      else if (error?.code === 'auth/invalid-email') errorMessage = 'รูปแบบอีเมลไม่ถูกต้อง';
      else if (error?.code === 'auth/user-disabled') errorMessage = 'บัญชีผู้ใช้ถูกปิดใช้งาน กรุณาติดต่อผู้ดูแลระบบ';
      else if (error?.code === 'auth/too-many-requests') errorMessage = 'คำขอเข้าสู่ระบบมากเกินไป กรุณารอสักครู่แล้วลองใหม่';
      setMsg(errorMessage);
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="login-page">
      <div className="login-shell">
        <div className="login-card">
          ...
          <p className="login-subtitle">กรุณาเข้าสู่ระบบเพื่อใช้งานระบบบริหารจัดการคลังสินค้า</p>

          <form onSubmit={handleLogin}>
            <div className="form-field">
              <label className="form-label" htmlFor="identifier">อีเมลหรือเบอร์โทรศัพท์</label>
              <input
                id="identifier"
            ...
            <div className="login-options">
              ...
              <a className="link-light" href="/forgot-password">ลืมรหัสผ่าน?</a>
            </div>
            ...
            <button className="login-button" type="submit" disabled={isLoading}>
              {isLoading ? (
                ...
                <svg className="icon-lock" viewBox="0 0 24 24" fill="currentColor">
            ...
            {msg && (
              <div className={`alert ${msg.includes('สำเร็จ') ? 'alert-success' : 'alert-error'}`}>
                {msg}
              </div>
            )}
          </form>
